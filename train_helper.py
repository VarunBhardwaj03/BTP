...
def model(input, reuse=False):
    ...
    with tf.name_scope("model"):
        with tf.compat.v1.variable_scope("conv1") as scope:
            ...
            net = tf.keras.layers.Conv2D(filters=256, kernel_size=(3, 3), activation='relu', 
                                         padding='same', kernel_initializer = tf.keras.initializers.GlorotUniform(seed=None))(input)
            # print(np.shape(net))
            # net = tf.layers.max_pooling2d(net, pool_size=2, strides=2, padding='valid')
            # net = tf.layers.batch_normalization(net, fused=True)
            # net = tf.nn.relu(net)
            print(np.shape(net))
            net=tf.keras.layers.MaxPool2D(pool_size=(2, 2),strides=2,padding='valid')(net)
        ...
        with tf.compat.v1.variable_scope("conv2") as scope:
            ...
            #                                weights_initializer=tf.contrib.layers.xavier_initializer_conv2d(),
            #                                scope=scope, reuse=reuse)
            net = tf.keras.layers.Conv2D(filters=128, kernel_size=(3, 3), activation='relu', 
                                         padding='same', kernel_initializer = tf.keras.initializers.GlorotUniform(seed=None))(net)
            # print(np.shape(net))
            # net = tf.layers.max_pooling2d(net, pool_size=2, strides=2, padding='valid')
            # net = tf.layers.batch_normalization(net, fused=True)
            # net = tf.nn.relu(net)
        ...
        with tf.compat.v1.variable_scope("conv3") as scope:
            ...
            #                                weights_initializer=tf.contrib.layers.xavier_initializer_conv2d(),
            #                                scope=scope, reuse=reuse)
            net = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), activation='relu', 
                                         padding='same', kernel_initializer = tf.keras.initializers.GlorotUniform(seed=None))(net)
            # print(np.shape(net))
            # net = tf.layers.max_pooling2d(net, pool_size=2, strides=2, padding='valid')
            # net = tf.layers.batch_normalization(net, fused=True)
        ...
        with tf.compat.v1.variable_scope("conv4") as scope:
            ...
            #                                weights_initializer=tf.contrib.layers.xavier_initializer_conv2d(),
            #                                scope=scope, reuse=reuse)
            net = tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 3), activation='relu', 
                                         padding='same', kernel_initializer = tf.keras.initializers.GlorotUniform(seed=None))(net)
            # print(np.shape(net))
            # net = tf.layers.max_pooling2d(net, pool_size=2, strides=2, padding='valid')
            # net = tf.layers.batch_normalization(net, fused=True)
        ...
        net = tf.keras.layers.Dense(4096, activation=tf.math.sigmoid)(net)
        print(np.shape(net))
...
def contrastive_loss(model1, model2, y, left_label, right_label, margin=0.2, use_loss=False):
    ...
    with tf.name_scope("contrastive_loss"):
        ...
        if use_loss:
            tf.compat.v1.losses.add_loss(similarity_loss)
...
def inference(left_input_image, right_input_image):
	...
	"""
    with tf.compat.v1.variable_scope('feature_generator', reuse=tf.compat.v1.AUTO_REUSE) as sc:

        left_features = model(tf.keras.layers.BatchNormalization()(tf.divide(left_input_image, 255.0)))
        
        right_features = model(tf.keras.layers.BatchNormalization()(tf.divide(right_input_image, 255.0)))
    ...
    logits = tf.keras.layers.Dense(units=1, activation=None)(merged_features)
    # net = tf.keras.layers.Dense(4096, activation=tf.math.sigmoid)(net)
...
def loss(logits, left_label, right_label):
    ...
    tf.compat.v1.losses.add_loss(cross_entropy_loss)
    
# def loss(logits, left_label, right_label):
#     label = tf.math.equal(left_label, right_label)
#     label_float = tf.cast(label, tf.float32)

#     logits_float = tf.cast(logits, tf.float32)
    
#     cross_entropy_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_float, labels=label_float))
    
#     # Add the computed loss to the current model
#     tf.keras.losses.add_loss(cross_entropy_loss)
