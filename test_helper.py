...
def model(input, reuse=False):
    ...
    with tf.name_scope("model"):
        with tf.compat.v1.variable_scope("conv1") as scope:
            ...
            # net = tf.keras.layers.max_pooling2d(net, pool_size=2, strides=2, padding='valid')
            net=tf.keras.layers.BatchNormalization()(net)
            # net = tf.keras.layers.batch_normalization(net, fused=True)
            net = tf.nn.relu(net)
            print(np.shape(net))
...
def inference(left_input_image, right_input_image):
    ...
    with tf.compat.v1.variable_scope('feature_generator', reuse=tf.compat.v1.AUTO_REUSE) as sc:
        mean1, variance1 = tf.nn.moments(tf.divide(left_input_image, 255.0), axes=[0])
        mean2, variance2 = tf.nn.moments(tf.divide(right_input_image, 255.0), axes=[0])
        left_features = model(tf.compat.v1.nn.batch_normalization(tf.divide(left_input_image, 255.0), mean1, variance1, 0.0, 1.0, 1e-5))
        right_features = model(tf.compat.v1.nn.batch_normalization(tf.divide(right_input_image, 255.0), mean2, variance2, 0.0, 1.0, 1e-5))
